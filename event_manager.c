...
void KeyboardManager(SDL_Event event, GameState *gameState, fighter *player,background* bg, Time *T){
		...
		if(player->p == STANDING || player->p == KNELT){
                	MovePlayerLeft(player, bg, T);
		}
        }

        if (keystate[SDLK_RIGHT]){
		if(player->p == STANDING || player->p == KNELT){
                	MovePlayerRight(player, bg, T);
		}
        }
...
void MovePlayerLeft(fighter *player, background *bg, Time *T){
...
void MovePlayerRight(fighter *player, background *bg, Time *T){
    update_currentTime(T);
    if(time_gap(*T) > TIME_BTW_ANIMATIONS){
...
/*handeling kicks*/
void AnimatePlayerKick(fighter *player, Time *T){
